...
class ClimbTestCase(TestCase):
    ...
    def test_climb_viewset_update_route(self):
        view = ClimbViewSet.as_view(actions={'put': 'update'})
        data = {
            'climb_name':"Villanueva de la concepción", 
            'peak_name':"El Torcal de Antquera",
        }
        request = self.factory.put('/api/climb', data=data)
        force_authenticate(request,user=self.user)
        response = view(request, pk=100, format='json')
        self.assertEqual(response.status_code, 200) 
        self.assertEqual(response.data['properties']['name'],'El Torcal de Antquera por Villanueva de la concepción')
